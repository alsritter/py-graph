    ...
    def execute(self, runners, runner_id, extra_data={}, execute_outputs=[]):
        ...
        while len(to_execute) > 0:
            ...
            to_execute = sorted(list(map(lambda a: (
                len(recursive_will_execute(runners, self.outputs, a[-1])), a[-1]), to_execute)))
...
def recursive_execute(server, runners, outputs, current_item, extra_data, executed, runner_id, outputs_ui, object_storage):
    ...
    for x in inputs:
        ...
        if isinstance(input_data, list):
            ...
            output_index = input_data[1]
            if input_unique_id not in outputs:
                result = recursive_execute(server, runners, outputs, input_unique_id, extra_data, executed, runner_id, outputs_ui, object_storage)
    ...
    input_data_all = None
    try:
        input_data_all = get_input_data(inputs, class_def, unique_id, outputs, runners, extra_data)
        if server.client_id is not None:
            server.last_node_id = unique_id
            server.send_sync("executing", { "node": unique_id, "runner_id": runner_id }, server.client_id)
        ...
        if len(output_ui) > 0:
            ...
            if server.client_id is not None:
                server.send_sync("executed", { "node": unique_id, "output": output_ui, "runner_id": runner_id }, server.client_id)
...
def recursive_will_execute(runners, outputs, current_item):
    ...
    for x in inputs:
        ...
        if isinstance(input_data, list):
            ...
            output_index = input_data[1]
            if input_unique_id not in outputs:
                will_execute += recursive_will_execute(runners, outputs, input_unique_id)
