...
app.registerExtension({
    ...
    init() {
        const editAttentionDelta = app.canvasManager.ui.settings.addSetting({
        ...
        function findNearestEnclosure(text, cursorPos) {
            ...
            while (start >= 0) {
                start--;
                if (text[start] === '(' && openCount === closeCount)
                    break;
                if (text[start] === '(')
                    openCount++;
                if (text[start] === ')')
            ...
            while (end < text.length) {
                if (text[end] === ')' && openCount === closeCount)
                    break;
                if (text[end] === '(')
                    openCount++;
                if (text[end] === ')')
        ...
        function editAttention(event) {
            ...
            if (!selectedText) {
                ...
                if (nearestEnclosure) {
                    ...
                    const delimiters = ' .,\\/!?%^*;:{}=-_`~()\r\n\t';
                    while (!delimiters.includes(inputField.value[start - 1]) &&
                        start > 0) {
                        start--;
                    }
                    while (!delimiters.includes(inputField.value[end]) &&
...
//# sourceMappingURL=editAttention.js.map