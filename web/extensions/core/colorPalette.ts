...
app.registerExtension({
  ...
  addCustomNodeDefs(node_defs, app) {
    ...
    const getCustomColorPalettes = () => {
      return app.canvasManager.ui.settings.getSettingValue(
    ...
    const setCustomColorPalettes = (customColorPalettes) => {
      return app.canvasManager.ui.settings.setSettingValue(
        idCustomColorPalettes,
        customColorPalettes
      )
    }
    ...
    const loadColorPalette = async (colorPalette) => {
      ...
      if (colorPalette.colors) {
        ...
        if (colorPalette.colors.node_slot) {
          Object.assign(
            app.canvasManager.canvas.default_connection_color_byType,
            colorPalette.colors.node_slot
          )
          Object.assign(
    ...
    const getColorPalette = (colorPaletteId?: string) => {
      if (!colorPaletteId) {
        colorPaletteId = app.canvasManager.ui.settings.getSettingValue(
          id,
    ...
    const setColorPalette = (colorPaletteId: string) => {
    ...
    app.canvasManager.ui.settings.addSetting({
      ...
      type: (name, setter, value) => {
        const options = [
          ...
          ...Object.values(getCustomColorPalettes()).map((c: any) =>
            $el('option', {
                ...
                $el('input', {
                  ...
                  onclick: async () => {
                    const colorPaletteId =
                      app.canvasManager.ui.settings.getSettingValue(
                        id,
                        defaultColorPaletteId
                      )
      ...
      async onChange(value) {
        ...
        app.canvasManager.canvas.updateBackground(
          BACKGROUND_IMAGE,
