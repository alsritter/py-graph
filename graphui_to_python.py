...
from output_utils import import_custom_nodes, find_path, get_value_at_index
...
class CodeGenerator:
    ...
    def assemble_python_code(self, import_statements: set, speical_functions_code: List[str], code: List[str], queue_size: int, custom_nodes=False) -> str:
        ...
        func_strings = []
        for func in [get_value_at_index, find_path]:
...
if __name__ == '__main__':
    ...
    # Convert GraphUI workflow to Python
    GraphUItoPython(input_file=input_file, output_file=output_file, queue_size=queue_size)